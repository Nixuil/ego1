...
public class PayHelper {
    ...
    public PayHelper(PayConfig payConfig) throws Exception {
    ...
    public String createPayUrl(Long orderId, String description, Long totalPay) {
        ...
        try {
            ...
            try {
                this.stringRedisTemplate.opsForValue().set(key, url, 2, TimeUnit.HOURS);
            } catch (Exception e) {
    ...
    private void isSignatureValid(Map<String, String> result) {
        try {
            ...
            if (!boo1 && !boo2) {
                throw new PayException(ExceptionEnum.WX_PAY_SIGN_INVALID);
            }
        } catch (Exception e) {
    ...
    public void handleNotify(Map<String, String> msg) {
        ...
        if (StringUtils.isBlank(totalFee) || StringUtils.isBlank(outTradeNo)
            ...
            throw new PayException(ExceptionEnum.WX_PAY_NOTIFY_PARAM_ERROR);
        }
        ...
        //判断支付状态
        OrderStatus orderStatus = statusMapper.selectById(Long.valueOf(outTradeNo));
        ...
        //修改支付日志状态
        PayLog payLog = payLogMapper.selectById(order.getOrderId());
        //未支付的订单才需要更改
        ...
        statusMapper.updateById(orderStatus1);
    }
